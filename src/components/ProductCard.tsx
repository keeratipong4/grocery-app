    ...
    <article className="w-[220px] flex-shrink-0 bg-white border border-border rounded-banner shadow-card overflow-hidden transition-all hover:shadow-md hover:-translate-y-1 group">
      <Link href={`/product/${product.id}`} className="block">
        ...
        <div className="flex flex-col gap-1 p-3 pb-0">
          ...
          <h3 className="text-sm font-medium text-gray-900 line-clamp-2 leading-snug group-hover:text-primary transition-colors">{product.name}</h3>
      ...
      <div className="p-3 pt-2">
        <button
          ...
          className="w-full bg-green-600 hover:bg-green-700 text-white text-sm font-semibold py-1.5 rounded-md transition-colors"
        >
          + เพิ่มในตะกร้า
        </button>
      </div>
